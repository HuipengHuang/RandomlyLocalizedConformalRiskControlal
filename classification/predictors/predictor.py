    ...
    def evaluate(self, cal_loader, test_loader):
        ...
        with torch.no_grad():
            ...
            if self.args.dataset == "imagenet":
                ...
                sscv_list[3] += torch.sum(set_size_coverage[7:11]).item() / (
                            torch.sum(set_size_num[7:11]).item() + 1e-6)
                my_set_size_num[3] = torch.sum(set_size_num[7:11]).item()

                sscv_list[4] += torch.sum(set_size_coverage[11:100]).item() / (
                            torch.sum(set_size_num[11:100]).item() + 1e-6)
                my_set_size_num[4] = torch.sum(set_size_num[11:100]).item()

                sscv_list[5] += torch.sum(set_size_coverage[100:]).item() / (
                            torch.sum(set_size_num[100:]).item() + 1e-6)
