...
class RandomlyLocalizedPredictor:
    ...
    def evaluate(self, cal_loader, test_loader):
        ...
        with torch.no_grad():
            ...
            target_test_score = test_score[torch.arange(test_target.shape[0]), test_target]


            p = self.kernel_function.get_p(cal_feature, test_feature, target_test_score, test_target)
            ...
            for i in range(prediction_set.shape[0]):
                ...
                set_size_coverage[torch.sum(prediction_set[i])] += prediction_set[i, test_target[i]].item()
